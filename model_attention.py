...
class Attention(object):
    ...
    def param_init_lstm_cond(self, options, params,
        ...
        Wc1_att = norm_weight(nin, nin)
        params[_p(prefix,'Wc1_att')] = Wc1_att

        # attention: LSTM -> hidden
        Wd_att = norm_weight(dim,nin)
        ...
        b1_att = numpy.zeros((nin,)).astype('float32')
        params[_p(prefix,'b1_att')] = b1_att

        # attention:
        U1_att = norm_weight(nin,1)
        params[_p(prefix,'U1_att')] = U1_att
        c1_att = numpy.zeros((1,)).astype('float32')
        params[_p(prefix, 'c1_tt')] = c1_att
        
        U1 = norm_weight(nin,dimctx)
        params[_p(prefix,'U1')] = U1
        c1 = norm_weight(dim,dimctx)
    ...
    def lstm_cond_layer(self, tparams, state_below, options, prefix='lstm',
                        mask=None, context=None, semantic=None,one_step=False,
        ...
        def _step(m_, x_, # sequences
                  h_, c_, beta_, so_, a_, ct_, # outputs_info
                  pso_, semantic, pctx_, ctx_, Wd_att, U1_att, c1_att, U1, c1, U_att, c_att, W_sel, b_sel, U, Wc, Wc1, # non_sequences
                  dp_=None, dp_att_=None):
            # textual attention
            pstate_ = tensor.dot(h_, Wd_att)
            pso_ = pso_ + pstate_[:, None, :]
            pso_ = tanh(pso_)
            
            beta = tensor.dot(pso_, U1_att)+c1_att
            beta = tensor.nnet.softmax(beta.reshape([beta.shape[0],beta.shape[1]])) # softmax
            so_ = (semantic * beta[:,:,None]).sum(1)
            ...
            # temporal attention
            sso_ = tensor.dot(so_, U1)
            pstate1_ = tensor.dot(h_, c1)
            pctx_ = pctx_ + pstate1_[:,None,:] + sso_[:,None,:]
            pctx_list = []
            ...
            h = o * tensor.tanh(c)
            h = m_[:,None] * h + (1. - m_)[:,None] * h_
            rval = [h, c, beta, so_, alpha, ctx_, pstate_, pctx_, i, f, o, preact, alpha_pre]+pctx_list
            return rval
        if options['use_dropout']:
            _step0 = lambda m_, x_, dp_, h_, c_, \
                beta_, so_, a_, ct_, \
                pso_, semantic, pctx_, context, Wd_att, U1_att, c1_att, U1, c1, U_att, c_att, W_sel, b_sel, U, Wc, Wc1: _step(
                m_, x_, h_, c_,
                beta_, so_, a_, ct_,
                pso_, semantic, pctx_, context, Wd_att, U1_att, c1_att, U1, c1, U_att, c_att, W_sel, b_sel, U, Wc, Wc1,  dp_)
        ...
        if one_step:
            if options['use_dropout']:
                rval = _step0(
                    mask, state_below, dp_mask, init_state, init_memory, None, None, None, None,
                    pso_, semantic, pctx_, context, Wd_att, U1_att, c1_att, U1, c1, U_att, c_att, W_sel, b_sel, U, Wc, Wc1)
            else:
            ...
            rval, updates = theano.scan(
                ...
                outputs_info = [init_state,
                                init_memory,
                                tensor.alloc(0., n_samples, pso_.shape[1]),
                                tensor.alloc(0., n_samples, semantic.shape[2]),
                                tensor.alloc(0., n_samples, pctx_.shape[1]),
                                tensor.alloc(0., n_samples, context.shape[2]),
                                None, None, None, None, None, None, None, None],
                                non_sequences=[pso_, semantic, pctx_, context,
                                               Wd_att, U1_att, c1_att, U1, c1, U_att, c_att, W_sel, b_sel, U, Wc, Wc1],
                                name=_p(prefix, '_layers'),
    ...
    def init_params(self, options):
        # all parameters
        params = OrderedDict()
        # embedding (training from scratch)
        params['Wemb'] = norm_weight(options['n_words'], options['dim_word'])
        # embedding with pre-trained word features such as glove and word2vec
        # pre_glove = numpy.load("/data/zhouchang/tyb/caption/PR/msvd/data/w2c_emb.npz")
        # glove_emb = pre_glove['w2c_emb'].astype('float32')
        # params['Wemb'] = glove_emb
        ...
        params = self.get_layer('ff')[0](
            options, params, prefix='ff_ctx', nin=ctx_dim, nout=ctx_dim)
        # decoder: LSTM
        ...
        if options['ctx2out']:
            ...
            params = self.get_layer('ff')[0](
                options, params, prefix='ff_logit_so',
    ...
    def build_model(self, tparams, options):
        ...
        s_o = tensor.matrix('s_o', dtype='int64')
        s_o.tag.test_value = self.s_o_tv
        mask_so = tensor.matrix('mask_s_o', dtype='float32')
        mask_so.tag.test_value = self.s_o_mask_tv
        ...
        # index semantic attributes into the embdding matrix
        s_o_emb = tparams['Wemb'][s_o.flatten()].reshape(
            [n_samples, 20, options['dim_word']])
        ...
        # ctx0 = self.get_layer('ff')[1](
        #     tparams, ctx0, options, prefix='ff_ctx', activ='rectifier')
        # decoder
        proj = self.get_layer('lstm_cond')[1](tparams, emb, options,
                                         prefix='decoder',
                                         mask=mask, context=ctx0, semantic=s_o0,
                                         one_step=False,
        ...
        if options['ctx2out']:
            ...
            # logit += self.get_layer('ff')[1](
            #     tparams, sos, options, prefix='ff_logit_so', activ='linear')
        logit = tanh(logit)
        if options['use_dropout']:
        ...
        extra = [probs, betas, alphas]
        return trng, use_noise, x, mask, s_o, mask_so, betas, ctx, mask_ctx, alphas, cost, extra
    ...
    def build_sampler(self, tparams, options, use_noise, trng, mode=None):
        ...
        so0 = tensor.vector('so_sampler', dtype='int64')
        so_mask = tensor.vector('so_mask', dtype='float32')
        
        # index semantic attributes into the embdding matrix
        s_o_emb = tparams['Wemb'][so0.flatten()].reshape(
            [20, options['dim_word']])
        ...
        init_memory = [self.get_layer('ff')[1](
            tparams, ctx_mean, options, prefix='ff_memory', activ='tanh')]
        # ctx = self.get_layer('ff')[1](
        #     tparams, ctx, options, prefix='ff_ctx', activ='rectifier')
        ctx = ctx.dimshuffle('x', 0, 1)
        s_o = s_o.dimshuffle('x', 0, 1)
        ...
        proj = self.get_layer('lstm_cond')[1](tparams, emb, options,
                                         prefix='decoder',
                                         mask=None, context=ctx, semantic=s_o,
                                         one_step=True,
                                         ...
                                         use_noise=use_noise,
                                         mode=mode)
        next_state, next_memory, sos, ctxs = [proj[0]], [proj[1]], [proj[3]], [proj[5]], 
        ...
        if options['ctx2out']:
            ...
            # logit += self.get_layer('ff')[1](
            #     tparams, sos[-1], options, prefix='ff_logit_so', activ='linear')
        logit = tanh(logit)
        if options['use_dropout']:
            logit = dropout_layer(logit, use_noise, trng)
        if options['n_layers_out'] > 1:
        ...
        f_next = theano.function(
            [x, ctx0, ctx_mask, so0, so_mask]+init_state+init_memory,
            [next_probs, next_sample]+next_state+next_memory,
            name='f_next', profile=False, mode=mode, on_unused_input='ignore')
        print 'Done'
        return f_init, f_next

    def gen_sample(self, tparams, f_init, f_next, ctx0, ctx_mask, so0, so_mask, options,
        ...
        for ii in xrange(maxlen):
            ...
            # next_state: [matrix]
            # next_memory: [matrix]
            rval = f_next(*([next_w, ctx0, ctx_mask, so0, so_mask]+next_state+next_memory))
            next_p = rval[0]
            if restrict_voc:
    ...
    def pred_probs(self, whichset, f_log_probs, verbose=True):
        ...
        for index in iterator:
            ...
            x, mask, ctx, ctx_mask, so, so_mask = data_engine.prepare_data(
                self.engine, tag)
            pred_probs = f_log_probs(x, mask, ctx, ctx_mask, so, so_mask)
    ...
    def train(self,
        ...
        [self.x_tv, self.mask_tv,
         self.ctx_tv, self.ctx_mask_tv, self.s_o_tv, self.s_o_mask_tv] = data_engine.prepare_data(
        ...
        trng, use_noise, \
              x, mask, so, mask_so, betas, ctx, mask_ctx, alphas,\
              cost, extra = \
        ...
        f_log_probs = theano.function([x, mask, ctx, mask_ctx, so, mask_so], -cost,
        ...
        if alpha_c > 0.:
            ...
            beta_reg = alpha_c * ((1.-betas.sum(0))**2).sum(0).mean()
            cost += alpha_reg
            cost += beta_reg
        ...
        if alpha_entropy_r > 0:
            ...
            beta_reg_2 = tensor.zeros_like(cost)
        print 'building f_alpha and f_beta'
        f_alpha = theano.function([x, mask, ctx, mask_ctx, so, mask_so],
                                  [alphas, alpha_reg_2],
                                  name='f_alpha',
                                  on_unused_input='ignore')
        f_beta = theano.function([x, mask, ctx, mask_ctx, so, mask_so],
                                  [betas, beta_reg_2],
                                  name='f_beta',
                                  on_unused_input='ignore')
        ...
        f_grad_shared, f_update = eval(optimizer)(lr, tparams, grads,
                                                  [x, mask, ctx, mask_ctx, so, mask_so], cost,
                                                  extra + grads)
        ...
        for eidx in xrange(max_epochs):
            ...
            for idx in self.engine.kf_train:
                ...
                ud_start = time.time()
                rvals = f_grad_shared(x, mask, ctx, ctx_mask, so, so_mask)
                cost = rvals[0]
                probs = rvals[1]
                betas = rvals[2]
                alphas = rvals[3]
                ...
                if numpy.mod(uidx, dispFreq) == 0:
                    ...
                    betas,beta_reg = f_beta(x,mask,ctx,ctx_mask,so,so_mask)
                    print 'beta ratio %.3f, beta_reg %.3f'%(
                        betas.min(-1).mean() / (betas.max(-1)).mean(), beta_reg)
                        
                    alphas,reg = f_alpha(x,mask,ctx,ctx_mask,so,so_mask)
                ...
                if numpy.mod(uidx, sampleFreq) == 0:
                    ...
                    def sample_execute(from_which):
                        ...
                        if from_which == 'train':
                            ...
                            x_s, mask_s, ctx_s, ctx_mask_s, so_s, so_mask_s = data_engine.prepare_data(
                                self.engine, tags)

                        stochastic = False
                        for jj in xrange(numpy.minimum(10, x_s.shape[1])):
                            sample, score, _, _ = self.gen_sample(
                                tparams, f_init, f_next, ctx_s[jj], ctx_mask_s[jj], so_s[jj], so_mask_s[jj],
                                model_options,
                                trng=trng, k=6, maxlen=30, stochastic=stochastic)
                            if not stochastic:
                ...
                if validFreq != -1 and numpy.mod(uidx, validFreq) == 0:
                    t0_valid = time.time()
                    betas,_ = f_beta(x, mask, ctx, ctx_mask, so, so_mask)
                    alphas,_ = f_alpha(x, mask, ctx, ctx_mask, so, so_mask)
