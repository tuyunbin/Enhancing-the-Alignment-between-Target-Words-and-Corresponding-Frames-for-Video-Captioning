...
def generate_sample_gpu_single_process(
    ...
    def sample(whichset):
        samples = []
        ctxs, ctx_masks, sos, so_masks = engine.prepare_data_for_blue(whichset)
        for i, ctx, ctx_mask, so, so_mask in zip(range(len(ctxs)), ctxs, ctx_masks, sos, so_masks):
            print 'sampling %d/%d'%(i,len(ctxs))
            sample, score, _, _ = model.gen_sample(
                None, f_init, f_next, ctx, ctx_mask, so, so_mask, options,
